...
class ILfUO_pusher_sim():
    def __init__(self,
        ...
        if self.regularize:
            self.results_path += '_w_reg'
        else:
            self.results_path += '_wo_reg'

        self.results_img_path = self.results_path + '/images'
        self.results_gif_path = self.results_path + '/gif'
        self.reuslts_log_path = self.results_path + "/logs/" + datetime.now().strftime("%Y%m%d-%H%M%S")
            ...
            for epoch in range(self.epochs):
                ...
                for step in range(bat_per_epo):
                    ...
                    with tf.GradientTape(persistent=True) as tape:
                        ...
                        recon_loss = tf.reduce_mean(tf.math.squared_difference(origin_vid, recon_vid))
                        z_c_loss = tf.reduce_mean(tf.math.squared_difference(z_c, z_c_origin))
                        z_m_loss = tf.reduce_mean(tf.math.squared_difference(z_m, z_m_origin))
                        z_loss = self.lambda_2 * (z_c_loss + z_m_loss)
                        enc_loss = tf.reduce_mean(tf.math.squared_difference(origin_frame, recon_frame))
