...
class PusherEnv3DOF(MujocoEnv, utils.EzPickle):
    ...
    def step(self, a):
        ...
        #     self.viewer.cam.azimuth += np.random.uniform(low=-2, high=2)
        # if hasattr(self, 'itr'):
        #     qpos = self.data.qpos
        #     qpos[:2] = 3*np.array([np.cos(self.itr*np.pi/100+self.view1_angle), -np.sin(self.itr*np.pi/100+self.view1_angle)])
