...
class PusherEnv3DOF_ILfUO(MujocoEnv, utils.EzPickle):
    def __init__(self, **kwargs):
        utils.EzPickle.__init__(self)
        self._kwargs = kwargs
        if 'enc_motion' in self._kwargs:
            self.enc_motion = self._kwargs['enc_motion']
            self.enc_content = self._kwargs['enc_content']
    ...
    def step(self, a):
        ...
        if 'demon_latent' in self._kwargs:
            if self.itr % 2 == 0:
                reward_latent = -np.sum((self._kwargs['demon_latent'][int(self.itr/2)]-ob[-(self._kwargs['dc']+self._kwargs['dm']):])**2)
            else:
                if self.itr == 49:
                    self.itr = 47
                reward_latent = -np.sum((self._kwargs['demon_latent'][int((self.itr+1)/2)] -ob[-(self._kwargs['dc']+self._kwargs['dm']):])**2)
    ...
    def _get_obs(self):
        ...
        if 'enc_motion' in self._kwargs:
            ...
            z_c, _ = self.enc_content(np.reshape(self.img_obs, (-1, 48, 48, 3)))
            z_m, _ = self.enc_motion(np.reshape(self.img_obs, (-1, 48, 48, 3)))
            z = tf.concat([z_c, z_m], axis=-1)
            obs = np.concatenate([
